        ...
        <div class="col-md-  8">
            <div class="card">
                ...
                @if(auth()->check() && ($shop->user_id == auth()->user()->id || auth()->user()->rol==0))
                    <a href="{{route('createProduct',['id'=>$shop->id])}}" class="btn btn-secondary btn-block mt-3">Añadir un nuevo producto</a>
                @endif
                    ...
                    <tbody> 
                        @forelse($shop->products as $product) 
                            <tr>
                                ...
                                <td>
                                    <a href="{{route('showProduct',['id'=>$product->id])}}" class="btn btn-primary">Ir al producto </a>
                                </td>
                                @if(auth()->check() && ($shop->user_id == auth()->user()->id || auth()->user()->rol==0))
                                    <td>
